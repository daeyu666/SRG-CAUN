...
class SpectralProjector(nn.Module):
    ...
    def msi_to_hsi(self, msi: torch.Tensor) -> torch.Tensor:
        # normalized transpose projection. It is only used as a weak spectral lifting prior.
...
class InitialReconstruction(nn.Module):
    def __init__(self, n_bands: int, n_msi_bands: int, hidden_dim: int, scale_ratio: int, srf_weights=None):
        ...
        self.in_proj = nn.Conv2d(n_bands * 3, hidden_dim, 3, padding=1)
    ...
    def forward(self, lr_hsi: torch.Tensor, hr_msi: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        msi_lift = self.projector.msi_to_hsi(hr_msi)
        residual_hint = msi_lift - lr_up
        feat = torch.cat([lr_up, msi_lift, residual_hint], dim=1)
        feat = self.body(self.in_proj(feat))
        pred = torch.clamp(lr_up + self.out_proj(feat), 0.0, 1.0)
        return pred, lr_up, msi_lift


# -----------------------------------------------------------------------------
# Shadow reliability estimator
# -----------------------------------------------------------------------------
class ShadowReliabilityEstimator(nn.Module):
    """
    输出连续可靠性图。数值越大表示越可信、越接近非阴影；数值越小表示阴影/低反射风险越高。
    """
    ...
    def forward(
        ...
        return {
            ...
            "boundary": boundary,
            "low_reflectance_risk": low_reflectance_risk,
        }


# -----------------------------------------------------------------------------
# Shadow-aware parameter predictor
# -----------------------------------------------------------------------------
class ShadowAwareParameterPredictor(nn.Module):
    ...
    def forward(
        ...
        lr_residual_map: torch.Tensor,
        msi_residual_map: torch.Tensor,
    ) -> Dict[str, torch.Tensor]:
        ...
        x = torch.cat([
            ...
            lr_residual_map,
            msi_residual_map,
            torch.abs(lr_residual_map - msi_residual_map),
        ], dim=1)
        raw = torch.sigmoid(self.net(x))

        # step is deliberately small for stable unfolding updates.
        ...
        # Hard-coded reliability tendency: non-shadow prefers MSI detail, shadow prefers HSI/reference prior.
...
class PhysicalConsistencyUpdate(nn.Module):
    ...
    def forward(
        ...
        z_next = z - params["step"] * grad
        z_next = torch.clamp(z_next, 0.0, 1.0)

        return z_next, {
            "lr_residual": lr_residual,
            "msi_residual": msi_residual,
            "lr_residual_map": lr_map,
            "msi_residual_map": msi_map,
        }


# -----------------------------------------------------------------------------
# Cross-scale non-shadow spectral reference matching
# -----------------------------------------------------------------------------
class NonShadowReferenceBank(nn.Module):
    """
    从当前场景内部的高可靠非阴影区域构建参考库。
    key 用于匹配，value 只保留光谱方向/趋势，不直接复制反射幅值。
    """
    ...
    def _match_one_scale(
        ...
        q = self.query_encoder(query_in).flatten(2).transpose(1, 2)  # B,N,D
        k = self.key_encoder(key_in).flatten(2).transpose(1, 2)      # B,N,D
        ...
        v = value_map.flatten(2).transpose(1, 2)                    # B,N,C

        ref_mask = (rel_s.flatten(2) > 0.55).float()                # B,1,N
        shadow_query_mask = (1.0 - rel_s).flatten(2).transpose(1, 2) # B,N,1

        sim = torch.bmm(q, k.transpose(1, 2))                       # B,N,N
        sim = sim.masked_fill(ref_mask <= 0.0, -1e4)
        ...
        matched = torch.sum(selected * attn.unsqueeze(-1), dim=2)    # B,N,C
    ...
    def forward(self, z: torch.Tensor, hr_msi: torch.Tensor, reliability: torch.Tensor) -> torch.Tensor:
        ...
        for scale, weight in zip([1, 2, 4], weights):
...
# -----------------------------------------------------------------------------
# Shadow-gated contourlet-like frequency prior
# -----------------------------------------------------------------------------
...
class ContourletRefinementPrior(nn.Module):
    ...
    def forward(
        ...
        # 阴影区域降低不可信高频；非阴影区域允许更多方向高频进入。
        gate_in = torch.cat([low, high, reliability, consistency_residual], dim=1)
        gate = self.gate(gate_in) * (0.25 + 0.75 * reliability)
        refined = low + gate * high
...
class SRGCAUNStage(nn.Module):
    ...
    def forward(
        ...
        z_next = z_data + params["w_prior"] * fused_delta + prior_delta
        z_next = torch.clamp(z_next, 0.0, 1.0)
...
class SRGCAUN(nn.Module):
    def __init__(self, cfg: SRGCAUNConfig):
        super().__init__()
        self.cfg = cfg
        self.initial = InitialReconstruction(
            n_bands=cfg.n_bands,
            n_msi_bands=cfg.n_msi_bands,
    ...
    def forward(self, lr_hsi: torch.Tensor, hr_msi: torch.Tensor, return_aux: bool = False):
        z, lr_up, msi_lift = self.initial(lr_hsi, hr_msi)
        ...
        for stage in self.stages:
            ...
            # 每轮用更新后的重建结果刷新可靠性图，使后续展开具备内容自适应性。
            msi_residual = self.projector.hsi_to_msi(z) - hr_msi
            msi_residual_lift = self.projector.msi_to_hsi(msi_residual)
            reliability_info = self.reliability_estimator(z, hr_msi, lr_residual_proxy, msi_residual_lift)
            reliability = reliability_info["reliability"]
